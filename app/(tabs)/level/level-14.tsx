...
const EGG_SPAWN_INTERVAL = 2000;

export default function LevelSecondScreen() {
  ...
  const spawnEgg = () => {
    ...
    const speed = 4 + Math.random() * 2; // 2-4
  ...
  const handleButtonPress = (color: 'blue' | 'red') => {
    ...
    setEggs(prev => prev.map(egg => {
      ...
      if (color === 'red' && conveyorIndex === 0) {
        const collectStart = screenWidth - 100;
        const collectEnd = screenWidth - 40;
        shouldCollect = egg.x >= collectStart && egg.x <= collectEnd;
      } else if (color === 'red' && conveyorIndex === 1) {
        shouldCollect = egg.x >= 30 && egg.x <= 110;
      } else if (color === 'blue' && conveyorIndex === 2) {
        shouldCollect = egg.x >= 60 && egg.x <= 140;
      }
  ...
  const renderRedPipe = () => {
    const pipeImage = require('@/assets/images/game/pipe-red-2.png')
  ...
  const renderBluePipe = () => {
    const pipeImage = require('@/assets/images/game/pipe-blue-2.png')
