...
export default function LevelScreen() {
  ...
  const handleButtonPress = (color: 'blue' | 'red') => {
    ...
    setEggs(prev => prev.map(egg => {
      ...
      if (color === 'red' && conveyorIndex === 0) {
        ...
        const collectStart = screenWidth - 140;
        const collectEnd = screenWidth - 60;
        shouldCollect = egg.x >= collectStart && egg.x <= collectEnd;
      } else if (color === 'blue' && conveyorIndex === 2) {
        const collectStart = screenWidth - 150;
        const collectEnd = screenWidth - 70;
        shouldCollect = egg.x >= collectStart && egg.x <= collectEnd;
      }
  ...
  const renderRedPipe = () => {
    const pipeImage = require('@/assets/images/game/pipe-red-2-mirror.png')
  ...
  const renderBluePipe = () => {
    const pipeImage = require('@/assets/images/game/pipe-blue-2-mirror.png')
