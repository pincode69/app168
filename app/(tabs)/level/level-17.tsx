...
export default function LevelScreen() {
  ...
  const handleButtonPress = (color: 'blue' | 'red') => {
    ...
    setEggs(prev => prev.map(egg => {
      ...
      if (color === 'red' && conveyorIndex === 0) {
        const collectStart = screenWidth - 160;
        const collectEnd = screenWidth - 80;
        shouldCollect = egg.x >= collectStart && egg.x <= collectEnd;
      } else if (color === 'blue' && conveyorIndex === 1) {
        const collectStart = screenWidth - 200;
        const collectEnd = screenWidth - 120;
        shouldCollect = egg.x >= collectStart && egg.x <= collectEnd;
  ...
  const renderRedPipe = () => {
    const pipeImage = require('@/assets/images/game/pipe-red.png')
  ...
  const renderBluePipe = () => {
    const pipeImage = require('@/assets/images/game/pipe-blue-mirror.png')
